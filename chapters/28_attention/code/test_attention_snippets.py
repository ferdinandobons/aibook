...
class AttentionSnippetTests(unittest.TestCase):
    def test_single_query_values(self) -> None:
        ...
        torch.testing.assert_close(
            scores,
            torch.tensor([0.70710678, 0.0, 0.70710678], dtype=torch.float64),
        )
        torch.testing.assert_close(
            weights,
            torch.tensor([0.40111209, 0.19777581, 0.40111209], dtype=torch.float64),
        )
        torch.testing.assert_close(
            output,
            torch.tensor([0.80222418, 0.59888791], dtype=torch.float64),
        )
