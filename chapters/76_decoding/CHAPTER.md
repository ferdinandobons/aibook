...
Qui decoding e generazione vincolata viene osservato come un meccanismo: il percorso va da «Greedy e beam search» a «Metriche». L'oggetto osservato è logits e spazio delle sequenze ammissibili. Il contratto locale dichiara input, logits, prefisso, temperatura e vincolo; operazione, greedy, beam, sampling, penalty e stop; output, token scelto, sequenza e metrica di costo. Il caso di partenza è Lo stesso vettore di logits produce un token greedy e un supporto di sampling espliciti. Il limite da non nascondere è: il decoding modifica la traiettoria, non corregge il modello a monte.
...
**Controllo.** Per «Greedy e beam search», scrivi il risultato atteso prima del calcolo, modifica una sola quantità e localizza il primo passaggio che cambia. Nel caso «Greedy e beam search», il vincolo da conservare è: Greedy sceglie il massimo locale; beam mantiene più prefissi secondo score accumulati e criteri di lunghezza.
...
**Controllo.** Per «Sampling», ricalcola il caso a mano e con lo snippet. Nel caso «Sampling», se i risultati divergono, confronta prima i valori intermedi e soltanto dopo l'output finale.


La relazione seguente è una mappa operativa e non una misura del sistema.

**Schema concettuale.** `y = decode(logits, constraint)`

Vincoli di decoding cambiano lo spazio delle sequenze ammissibili. [SRC-76-001]
...
**Controllo.** Per «Penalità e stop», aggiungi un valore limite e verifica separatamente forma, valore e ipotesi. Una shape valida non dimostra da sola «Penalità e stop».
...
**Controllo.** Per «Constrained decoding», mantieni fisso l'input e sostituisci soltanto il meccanismo discusso nella sezione. Nel caso «Constrained decoding», il confronto deve attribuire la differenza a quel passaggio, non al setup.


## Esempio Python eseguito

Per rendere osservabile decoding e generazione vincolata, il capitolo conserva qui l'artefatto Python eseguito. Per «Decoding e generazione vincolata», il caso di default usa valori piccoli per isolare il meccanismo. Il test rifiuta anche un caso non documentato di «decoding e generazione vincolata».
...
**Controllo.** Per «Metriche», costruisci un controesempio che rispetti il tipo di dato ma violi l'ipotesi centrale. Il test deve rendere riconoscibile perché «Metriche» non si applica.
...
- **Da «Greedy e beam search» a «Sampling».** Greedy sceglie il massimo locale; beam mantiene più prefissi secondo score accumulati e criteri di lunghezza. Temperature, top-k e top-p modificano la distribuzione prima dell'estrazione. Tra «Greedy e beam search» e «Sampling» l'ingresso viene fissato prima della regola che produce il valore. Il passaggio successivo rende misurabile «Sampling». [SRC-76-001; SRC-76-002]

- **Da «Sampling» a «Penalità e stop».** Temperature, top-k e top-p modificano la distribuzione prima dell'estrazione. Repetition penalty, stop sequence e minimum length intervengono in punti differenti e possono interagire. Nel caso «Penalità e stop» il componente diventa il punto in cui localizzare l'errore. Da «Sampling» a «Penalità e stop» cambia la domanda osservabile. [SRC-76-002; SRC-76-003]

- **Da «Penalità e stop» a «Constrained decoding».** Repetition penalty, stop sequence e minimum length intervengono in punti differenti e possono interagire. Grammar, automi e schema limitano i token ammessi. Dopo «Penalità e stop», la variante di «Constrained decoding» cambia una proprietà alla volta. Il passaggio successivo rende misurabile «Constrained decoding». [SRC-76-003; SRC-76-004]

- **Da «Constrained decoding» a «Metriche».** Grammar, automi e schema limitano i token ammessi. Qualità, diversità, latency, token per secondo e probabilità della sequenza devono essere letti insieme. Da «Metriche» in poi la misura resta distinta dalla correttezza locale del calcolo. Da «Constrained decoding» a «Metriche» cambia la domanda osservabile. [SRC-76-004; SRC-76-001]
